"""
test_roundtrip.py  (lighting_control fixture)

Seam-2 round-trip proof for the host DLL. PASSED 2026-08-11: harness_init() -> 0,
button04 press flips the decoded lights byte 0x01 -> 0x10 through the full
bridge -> gt_svg -> gt_can_devs -> update_lightControl -> gt_svg -> bridge path.
ctypes is used here for the proof (production target is cffi API-mode); the
point was only to prove the persistent-handle DLL + Seam-2 transfer work
together on a real static build.

Run under Windows Python (AppData/.../Python312), NOT the MinGW shell.

    python test_roundtrip.py

Expects lighting_control.dll in the same folder. Build command: see the
project's Seam-2 EDR (reqs/EDR_seam2_lighting_control.md).
"""
